...
Use this when you want to connect to a polarish server from another device such as a phone, tablet, or separate desktop app.
...
with the same server-side behavior as `agent serve --tailscale-serve`, then the server asks Tailscale
...
Run the server with `agent serve`.

```bash
npx @polarish/agent serve --host "$(tailscale ip -4)"
```

`agent serve` starts the server without opening a browser and prints:
...
Use `agent serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx @polarish/agent serve --tailscale-serve
```
...
```bash
npx @polarish/agent serve --tailscale-serve --tailscale-serve-port 8443
```

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `agent project ...` on the server machine instead.
> Full GUI support for remote project management is coming soon.

### Option 3: Desktop-Managed SSH Launch

Use this when you want the desktop app to start or reuse polarish on another machine over SSH.
...
1. `agent serve` issues a one-time owner pairing token.
...
Hosted pairing does not proxy traffic through polarish. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `agent auth` to manage access after the initial pairing flow.
...
Use `agent auth --help` and the nested subcommand help pages for the full reference.
...
- Use `agent auth` to revoke credentials or sessions you no longer trust.
