...
This guide is for people who want to use more than one Codex account in polarish.
...
In polarish Settings, name it something obvious:
...
In polarish Settings, add another Codex provider:
...
polarish shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, polarish treats it
