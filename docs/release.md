...
- Publishes the CLI package (`apps/server`, npm package `@polarish/agent`) with OIDC trusted publishing from the same workflow file:
...
- Publishes the CLI package (`apps/server`, npm package `@polarish/agent`) to the `nightly` npm dist-tag using the same nightly version.
...
1. Confirm the npm org/user owns package `@polarish/agent` (create it under the org if needed).
