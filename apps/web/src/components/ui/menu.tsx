  ...
  return (
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          className={cn(
            "relative flex not-[class*='w-']:min-w-32 origin-(--transform-origin) rounded-lg border bg-popover not-dark:bg-clip-padding shadow-lg/5 outline-none transition-[scale,opacity] duration-(--dropdown-open-dur) ease-(--dropdown-ease) will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] data-ending-style:scale-(--dropdown-closing-scale) data-starting-style:scale-(--dropdown-pre-scale) data-ending-style:opacity-0 data-starting-style:opacity-0 motion-reduce:transition-none focus:outline-none dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            className,
          )}
