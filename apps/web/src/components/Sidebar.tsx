...
const SidebarChromeHeader = memo(function SidebarChromeHeader({
  ...
  return isElectron ? (
    <SidebarHeader className="drag-region h-[52px] shrink-0 flex-row items-center gap-2 px-4 py-0 pl-[90px] wco:h-[env(titlebar-area-height)] wco:pl-[calc(env(titlebar-area-x)+1em)]">
      {wordmark}
    </SidebarHeader>
  ) : (
    <SidebarHeader className="shrink-0 gap-3 px-3 py-2 sm:gap-2.5 sm:px-4 sm:py-3">
      {wordmark}
    </SidebarHeader>
  );
});
